...
const hasGPS = item => number(item.lat) && number(item.lon) && Math.abs(Number(item.lat))<=90 && Math.abs(Number(item.lon))<=180;
const reading = value => number(value) ? String(value) : '—';
function make(tag,text,cls){const e=document.createElement(tag);if(text!==undefined)e.textContent=String(text);if(cls)e.className=cls;return e;}
function details(title,record){$('detail-title').textContent=title;$('detail-fields').replaceChildren();Object.entries(record).forEach(([key,value])=>$('detail-fields').append(make('dt',key.replaceAll('_',' ')),make('dd',value===null?'Unavailable':typeof value==='object'?JSON.stringify(value,null,2):value)));if(!$('detail-dialog').open)$('detail-dialog').showModal();}
function inspectable(element,title,record){element.tabIndex=0;element.setAttribute('role','button');element.setAttribute('aria-label','Inspect '+title);const open=()=>details(title,record);element.addEventListener('click',open);element.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open();}});}
function empty(root,title,description){const e=make('div',undefined,'empty');e.append(make('strong',title),make('p',description));root.replaceChildren(e);}
...
$('operator-note').addEventListener('input',()=>{try{localStorage.setItem('citadel-operator-note',$('operator-note').value);$('note-status').textContent='Saved locally';}catch{$('note-status').textContent='Local storage unavailable';}});
let mapZoom=1;function setMapZoom(value){mapZoom=Math.max(1,Math.min(3,value));$('map-viewport').classList.remove('zoom-2','zoom-3');if(mapZoom>1)$('map-viewport').classList.add('zoom-'+mapZoom);$('map-zoom').textContent=mapZoom+'×';$('map-in').disabled=mapZoom===3;$('map-out').disabled=mapZoom===1;}
$('map-in').addEventListener('click',()=>setMapZoom(mapZoom+1));$('map-out').addEventListener('click',()=>setMapZoom(mapZoom-1));$('map-reset').addEventListener('click',()=>setMapZoom(1));$('map-layer').addEventListener('change',()=>$('map-viewport').classList.toggle('grayscale',$('map-layer').value==='grayscale'));
setMapZoom(1);setPage();updateClock();window.addEventListener('hashchange',setPage);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh();});setInterval(updateClock,1000);setInterval(()=>{if(!document.hidden)refresh();},3000);refresh();
